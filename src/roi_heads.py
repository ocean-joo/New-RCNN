...
def project_masks_on_boxes(gt_masks, boxes, matched_idxs, M):
    ...
    return roi_align(gt_masks, rois, M, 1.)[:, 0]
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    temp_size = (mask_logits.shape[-2], mask_logits.shape[-1])
    # print(mask_matched_idxs)
    labels = [l[idxs] for l, idxs in zip(gt_labels, mask_matched_idxs)]
    mask_targets = [
        # project_masks_on_boxes(m, p, i, discretization_size)
        project_masks_on_boxes(m, p, i, temp_size)
...
def keypoints_to_heatmap(keypoints, rois, heatmap_size):
    ...
    x[x_boundary_inds] = torch.tensor(heatmap_size[1] - 1)
    y[y_boundary_inds] = torch.tensor(heatmap_size[0] - 1)

    valid_loc = (x >= 0) & (y >= 0) & (x < heatmap_size[1]) & (y < heatmap_size[0])
    vis = keypoints[..., 2] > 0
    valid = (valid_loc & vis).long()

    lin_ind = y * heatmap_size[0] + x
...
class RoIHeads(torch.nn.Module):
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        # if 1 in targets[0]['labels'] and self.has_mask() \
        if self.has_mask() \
                and self.keypoint_roi_pool is not None and self.keypoint_head is not None \
                and self.keypoint_predictor is not None:
            # segmentation
            mask_key_proposals = [p["boxes"] for p in result]
            ...
            if self.training:
                ...
                for img_id in range(num_images):
                    ...
                    mask_key_proposals.append(proposals[img_id][pos])
                    pos_matched_idxs.append(matched_idxs[img_id][pos])
            else:
                pos_matched_idxs = None

            if self.mask_roi_pool is not None:
                mask_features = self.mask_roi_pool(features, mask_key_proposals, image_shapes)
            ...
            # keypoint_features = self.keypoint_roi_pool(features, mask_key_proposals, image_shapes)
            keypoint_features = self.keypoint_head(mask_features)
            ...
            if self.training:
                ...
                rcnn_loss_mask = maskrcnn_loss(
                    mask_logits, mask_key_proposals,
                    gt_masks, gt_labels, pos_matched_idxs)
                rcnn_loss_keypoint = keypointrcnn_loss(
                    keypoint_logits, mask_key_proposals,
                ...
                assert mask_key_proposals is not None
                ...
                keypoints_probs, kp_scores = keypointrcnn_inference(keypoint_logits, mask_key_proposals)
        ...
        '''
            ...
            if self.keypoint_roi_pool is not None and self.keypoint_head is not None \
                ...
                # Add Mask Features to Keypoint Proposal
                keypoint_features = torch.cat((keypoint_features, self.mask_features), 1)
